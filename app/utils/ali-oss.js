...
function aliOSS(options) {
    ...
    this.defaults = {
        ...
        bucket: AppConfig.ali_oss.bucket || 'test-goods-item-images'
    };
...
aliOSS.prototype.uploadFile = function (args, options) {
    ...
    form.multiples = false;    //设置是否支持多文件上传，下面代码未处理该逻辑
    ...
    form.parse(req, function (err, fields, files) {
        ...
        fs.rename(filePath, newPath, function (err) {
            ...
            co(function* () {
                ...
                var result = yield client.putStream(me.config.ossDir + newName, stream);
...
aliOSS.prototype.uploadImage = function (args) {
    ...
    if (session.user) {
        ...
        return res.json({error_code: -1, msg: '未登录或登录信息已过期'});
    }
...
aliOSS.prototype.showBucket = function (args) {
    var me = this, req = args[0], res = args[1];

    var client = new OSS(me.config);
    ...
    co(function* () {
        ...
        result.fileList = yield client.list({'max-keys': 1000});
