...
(function ($) {
    ...
    var Picker = function (options) {
        ...
        var defaults = {
            ...
            onConfirm: null, //点击确定按钮关闭之后执行的回调
            inputReadOnly: true, //input是否只读
            toolbarHtml: null, //头部自定义
            toolbarTitle: '请选择',  //标题
            toolbarCloseText: '取消',  //关闭按钮文案
            toolbarConfirmText: '确认'  //确认按钮文案
        };
        ...
        /*头部dom结构*/
        params.toolbarTpl = params.toolbarHtml || [
                '<header class="tool-bar">',
                (params.toolbarCloseText ? '<button class="close-picker">' + params.toolbarCloseText + '</button>' : ''),
                '<h1 class="title">' + params.toolbarTitle + '</h1>',
                '<button class="confirm-picker">' + params.toolbarConfirmText + '</button>',
                '</header>'
        ].join('');
        ...
        function modalClose(modal, action, callback) {
            ...
            if (action === 'close' || params.ifClose.call(self, result)) {
                var thisModal = $(modal);
                self.opened = false;

                thisModal.addClass('modal-out').removeClass('modal-in');
                if (elements.mask) {
                    elements.mask.stop().fadeOut(function () {
        ...
        self.open = function () {
            if (!self.opened) {
                ...
                if (!self.inline) {
                    ...
                    if (params.mask) {
                        elements.mask = $('<div class="picker-mask"></div>').appendTo(params.container || elements.body);
                    }
                    elements.cols = elements.modal.find('.picker-items-col');

                    /*关闭后的回调函数*/
                    elements.modal.on('close', function () {
                        modalClose(this, 'close', function (input, res) {
                    ...
                    elements.modal.on('confirm', function () {
                        if (!params.atOnce && params.onConfirm) {
                            modalClose(this, 'confirm', function (input, res) {
